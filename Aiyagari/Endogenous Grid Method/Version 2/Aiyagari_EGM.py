...
markets and idiosyncratic income risk as in Aiyagari (1994) using the endogenous grid method. The continuous income process 
...
class AiyagariEGM:
    ...
    def __init__(self, a_bar = 0,              #select borrowing limit
        ...
        self.params_egm = self.beta, self.pi, self.grid_sav, self.grid_z, self.sigma, self.maxit_hh, self.tol_hh
        
        if self.distribution_method == 'discrete' or self.plot_supply_demand:
        ...
        if self.distribution_method == 'monte carlo':
            self.params_sim = self.a0, self.simN, self.simT, self.grid_z, self.grid_sav, self.sigma, self.beta, self.pi, self.shock_history
    ...
    def graph_supply_demand(self):
        ...
        *Output
        ...
        for idx, r_graph in enumerate(self.r_vec):
            ...
            # d. solve economy
            # i. household problem
            _, pol_sav_graph, a_star_graph, _ = solve_hh(self.params_egm, r_graph, w_graph)
    ...
    def ge_algorithm(self, r_ss_guess):
        ...
        self.pol_cons, self.pol_sav, self.a_star, self.it_hh = solve_hh(self.params_egm, r_ss, w_ss)
...
############################################
# 2. Household and Endogenous Grid Method #
###########################################

@njit   
def solve_hh(params_egm, r, w):
    ...
    *Output
    ...
    beta, pi, grid_sav, grid_z, sigma, maxit_hh, tol_hh = params_egm
    ...
    for it_hh in range(maxit_hh):
        
        # i. iterate
        pol_cons, a_star = egm_algo(params_egm, pol_cons_old, r, w)
...
@njit
def egm_algo(params_egm, pol_cons_old, r, w):
...
def simulate_MonteCarlo(pol_cons, pol_sav, r, w, params_sim):
    ...
    *Output
    ...
    # a. initialization
    a0, simN, simT, grid_z, grid_sav, sigma, beta, pi, shock_history = params_sim
