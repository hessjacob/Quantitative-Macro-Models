...
parameters sigma alpha beta eta theta epsilon rho_a rho_z rho_R rho_nu psi phi_pi phi_y
...
%(17) Monetary policy
log(R) = (1-rho_R)*log(R_ss) + rho_R*log(R(-1)) + (1-rho_R)*(phi_pi*log(Pi/Pi_ss) + phi_y*log(Y/Y_ss)) + nu;
%R=1/betta*(Pi/steady_state(Pi)^phi_pi*(Y/steady_state(Y))^phi_y*exp(nu); 
...
shocks;
var eNU;    stderr 0.0025;  %1 standard deviation shock of 25 basis points, i.e. 1 percentage point annualized
                    
var eZ;     stderr 0.02;    % intertemporal preference shock. (from Sims NGDP)
                            % It doesn?t impact you value utility from consumption versus utility from leisure, 
                            % but rather how you value utility today versus utility in the future.

var eA;     stderr 0.01;    %unit shock to technology
...
stoch_simul(order=1,irf=20,nograph) Y Pi R N yf log_A log_Z nu;
