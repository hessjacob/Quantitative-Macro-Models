...
in partial equilibrium using the endogenous grid method. Unlike the prior versions the EGM method here is from Alisdair 
McKay's notes and is a different variant. In addition, it runs a simulation, computes the invariant distribution and 
calculates the euler equation error over the simulation.
...
class ConSaveEGMV3:
    ...
    def setup_parameters(self):
        ...
        # savings grid
        # 1) number of of grid points: Ideally have a low number like 30-50 when using exponential grid and this EGM variant.
        # 2) if you need many grid points (100+) an exponential grid could return unstable solutions using this EGM variant. 
    ...
    def solve_model(self):
        ...
        if self.plott:
            ...
            plt.savefig('savings_policyfunction_egmv3.pdf')
            plt.show()
            ...
            plt.savefig('consumption_policyfunction_egmv3.pdf')
            plt.show()
            ...
            plt.savefig('simulation_egmv3.pdf')
            plt.show()
            ...
            plt.ylabel('Density')
            plt.legend(['z='+str(self.grid_z[0]),'z='+str(self.grid_z[1])])
            plt.savefig('joint_density_egmv3.pdf')
            plt.show()
            ...
            plt.savefig('wealth_density_egmv3.pdf')
            plt.show()
